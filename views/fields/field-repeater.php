...
?><ul class="repeaters"><?php
	...
	for ( $i = 0; $i <= $count; $i++ ) {
		...
		?><li class="<?php echo esc_attr( $setatts['class'] ); ?>">
			<div class="handle closed">
				<span class="title-repeater"><?php echo esc_html( $setatts['label-header'], 'eggz-reservations' ); ?></span>
				<button aria-expanded="true" class="btn-edit" type="button">
					<span class="screen-reader-text"><?php echo esc_html( $setatts['label-edit'], 'eggz-reservations' ); ?></span>
				...
				<div class="wrap-fields"><?php
					...
					foreach ( $setatts['fields'] as $fieldcount => $field ) {
						...
						foreach ( $field as $type => $atts ) {
							...
							include( plugin_dir_path( dirname( __FILE__ ) ) . 'fields/field-' . $type . '.php' );
				...
				<div>
					<a class="link-remove" href="#">
						<span><?php
							...
							echo esc_html( apply_filters( $this->plugin_name . '-repeater-remove-link-label', $setatts['label-remove'] ), 'eggz-reservations' );
	...
	?><div class="repeater-more">
		...
		<a class="button" href="#" id="add-repeater"><?php
			...
			echo esc_html( apply_filters( $this->plugin_name . '-repeater-more-link-label', $setatts['label-add'] ), 'eggz-reservations' );
